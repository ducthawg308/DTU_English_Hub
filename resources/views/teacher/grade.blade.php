...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ route('teacher.combined.submit-grade', $studentInfo->user_id) }}" method="POST">
                @csrf
...
<script>
    document.querySelector('form').addEventListener('submit', function(e) {
        // Populate writing data
        @foreach($writingSubmissions as $writingSubmission)
            if (document.getElementById('writing_teacher_score_{{ $writingSubmission->response_id }}')) {
                const writingScoreInput = document.createElement('input');
                writingScoreInput.type = 'hidden';
                writingScoreInput.name = 'writing_teacher_scores[]';
                writingScoreInput.value = document.getElementById('writing_teacher_score_{{ $writingSubmission->response_id }}').value;
                this.appendChild(writingScoreInput);
                
                const writingFeedbackInput = document.createElement('input');
                writingFeedbackInput.type = 'hidden';
                writingFeedbackInput.name = 'writing_teacher_feedbacks[]';
                writingFeedbackInput.value = document.getElementById('writing_teacher_feedback_{{ $writingSubmission->response_id }}').value;
                this.appendChild(writingFeedbackInput);
            }
        @endforeach
        
        // Populate speaking data
        @foreach($speakingSubmissions as $speakingSubmission)
            if (document.getElementById('speaking_teacher_score_{{ $speakingSubmission->response_id }}')) {
                const speakingScoreInput = document.createElement('input');
                speakingScoreInput.type = 'hidden';
                speakingScoreInput.name = 'speaking_teacher_scores[]';
                speakingScoreInput.value = document.getElementById('speaking_teacher_score_{{ $speakingSubmission->response_id }}').value;
                this.appendChild(speakingScoreInput);
                
                const speakingFeedbackInput = document.createElement('input');
                speakingFeedbackInput.type = 'hidden';
                speakingFeedbackInput.name = 'speaking_teacher_feedbacks[]';
                speakingFeedbackInput.value = document.getElementById('speaking_teacher_feedback_{{ $speakingSubmission->response_id }}').value;
                this.appendChild(speakingFeedbackInput);
            }
        @endforeach
    });
</script>
