        ...
        <div class="card">
            ...
            <form id="listeningForm">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <div class="radio-group">
                            <div class="radio-item">
                                <input type="radio" id="level_a2" name="level" value="A2" 
                                    {{ (old('level', $level ?? 'B1') == 'A2') ? 'checked' : '' }} required>
                                <label for="level_a2">A2 - Cơ bản</label>
                            </div>
                            <div class="radio-item">
                                <input type="radio" id="level_b1" name="level" value="B1" 
                                    {{ (old('level', $level ?? 'B1') == 'B1') ? 'checked' : '' }} required>
                                <label for="level_b1">B1 - Trung cấp</label>
                            </div>
                            <div class="radio-item">
                                <input type="radio" id="level_b2" name="level" value="B2" 
                                    {{ (old('level', $level ?? 'B1') == 'B2') ? 'checked' : '' }} required>
                                <label for="level_b2">B2 - Trung cấp cao</label>
                            </div>
                            <div class="radio-item">
                                <input type="radio" id="level_c1" name="level" value="C1" 
                                    {{ (old('level', $level ?? 'B1') == 'C1') ? 'checked' : '' }} required>
                    ...
                    <!-- Chủ đề mong muốn -->
                    @php
                        $fixedTopics = ['daily_communication', 'academic', 'workplace', 'travel', 'mixed'];
                        $topicValue = old('topic', $topic ?? 'mixed');
                        $customValue = old('custom_topic_text', (!in_array($topicValue, $fixedTopics) ? $topicValue : ''));
                        $isCustom = !in_array($topicValue, $fixedTopics);
                    @endphp
                    ...
                    <div class="form-group">
                        ...
                        <select id="topic" name="topic" class="form-control">
                            <option value="daily_communication" {{ $topicValue == 'daily_communication' ? 'selected' : '' }}>Giao tiếp hàng ngày</option>
                            <option value="academic" {{ $topicValue == 'academic' ? 'selected' : '' }}>Trường học / Học thuật</option>
                            <option value="workplace" {{ $topicValue == 'workplace' ? 'selected' : '' }}>Công việc / Văn phòng</option>
                            <option value="travel" {{ $topicValue == 'travel' ? 'selected' : '' }}>Du lịch</option>
                            <option value="mixed" {{ $topicValue == 'mixed' ? 'selected' : '' }}>Không giới hạn (Tổng hợp)</option>
                            <option value="custom" {{ $isCustom ? 'selected' : '' }}>Tự chọn chủ đề</option>
                        </select>

                        <div id="custom_topic" class="custom-topic" style="{{ $isCustom ? '' : 'display:none;' }}">
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            ...
                            const customDiv = document.getElementById('custom_topic');
                            ...
                            topicSelect.addEventListener('change', toggleCustomInput);
                            toggleCustomInput(); // chạy lần đầu khi load trang
                        });
                    </script>
